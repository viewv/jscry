...
function(t, e, r) {
        e.readUInt32BE = function(t, e) {
            ...
            for (var i = 0, o = 0, a = 0; a < 4; a++) for (var s = 24; 0 <= s; s -= 8) i <<= 1, 
            i |= e >>> s + a & 1, i <<= 1, i |= t >>> s + a & 1;
            for (a = 4; a < 8; a++) for (s = 24; 0 <= s; s -= 8) o <<= 1, o |= e >>> s + a & 1, 
            o <<= 1, o |= t >>> s + a & 1;
            r[n + 0] = i >>> 0, r[n + 1] = o >>> 0;
        ...
        e.pc2 = function(t, e, r, n) {
            for (var i = 0, o = 0, a = c.length >>> 1, s = 0; s < a; s++) i <<= 1, i |= t >>> c[s] & 1;
            for (s = a; s < c.length; s++) o <<= 1, o |= e >>> c[s] & 1;
            r[n + 0] = i >>> 0, r[n + 1] = o >>> 0;
        }, e.expand = function(t, e, r) {
            var n = 0, i = 0;
            n = (1 & t) << 5 | t >>> 27;
            for (var o = 23; 15 <= o; o -= 4) n <<= 6, n |= t >>> o & 63;
            for (o = 11; 3 <= o; o -= 4) i |= t >>> o & 63, i <<= 6;
        ...
        e.substitute = function(t, e) {
            for (var r = 0, n = 0; n < 4; n++) {
                var i = t >>> 18 - 6 * n & 63, o = a[64 * n + i];
                r <<= 4, r |= o;
            }
            for (n = 0; n < 4; n++) r <<= 4, r |= o = a[256 + 64 * n + (i = e >>> 18 - 6 * n & 63)];
            return r >>> 0;
        };
...
// ==================== 元数据 ====================
// 此文件包含从 wx0526b0432a899356 提取的 DES 算法实现
// 检测位置: 行 5914-5914
// 变量名: a
// 检测源: static
