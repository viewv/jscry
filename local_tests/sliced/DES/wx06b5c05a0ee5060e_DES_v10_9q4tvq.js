 ...
 * 检测类型: Permutation table
 * 置信度: 0.85
 * 函数名: anonymous
 * 行数: 77
 * 生成时间: 2025-07-05T13:17:10.731Z
 */
...
function(t, e, r) {
    ...
    e.readUInt32BE = function(t, e) {
        ...
        for (var i = 0, o = 0, a = 6; a >= 0; a -= 2) {
            for (var c = 0; c <= 24; c += 8) i <<= 1, i |= e >>> c + a & 1;
            for (c = 0; c <= 24; c += 8) i <<= 1, i |= t >>> c + a & 1;
        }
        for (a = 6; a >= 0; a -= 2) {
            for (c = 1; c <= 25; c += 8) o <<= 1, o |= e >>> c + a & 1;
            for (c = 1; c <= 25; c += 8) o <<= 1, o |= t >>> c + a & 1;
        }
        r[n + 0] = i >>> 0, r[n + 1] = o >>> 0;
    }, e.rip = function(t, e, r, n) {
        for (var i = 0, o = 0, a = 0; a < 4; a++) for (var c = 24; c >= 0; c -= 8) i <<= 1, 
        i |= e >>> c + a & 1, i <<= 1, i |= t >>> c + a & 1;
        for (a = 4; a < 8; a++) for (c = 24; c >= 0; c -= 8) o <<= 1, o |= e >>> c + a & 1, 
        o <<= 1, o |= t >>> c + a & 1;
        r[n + 0] = i >>> 0, r[n + 1] = o >>> 0;
        ...
        for (c = 0; c <= 24; c += 8) i <<= 1, i |= e >> c + a & 1;
        for (a = 1; a <= 3; a++) {
            for (c = 0; c <= 24; c += 8) o <<= 1, o |= e >> c + a & 1;
            for (c = 0; c <= 24; c += 8) o <<= 1, o |= t >> c + a & 1;
        }
        for (c = 0; c <= 24; c += 8) o <<= 1, o |= t >> c + a & 1;
        r[n + 0] = i >>> 0, r[n + 1] = o >>> 0;
    ...
    e.pc2 = function(t, e, r, i) {
        for (var o = 0, a = 0, c = n.length >>> 1, s = 0; s < c; s++) o <<= 1, o |= t >>> n[s] & 1;
        for (s = c; s < n.length; s++) a <<= 1, a |= e >>> n[s] & 1;
        r[i + 0] = o >>> 0, r[i + 1] = a >>> 0;
...
// ==================== 元数据 ====================
// 此文件包含从 wx06b5c05a0ee5060e 提取的 DES 算法实现
// 检测位置: 行 11559-11559
// 变量名: o
// 检测源: static
