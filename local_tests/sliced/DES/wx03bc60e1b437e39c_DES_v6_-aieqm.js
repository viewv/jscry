...
function(e, t, n) {
        ...
        t.readUInt32BE = function(e, t) {
            ...
            for (var i = 0, a = 0, s = 6; s >= 0; s -= 2) {
                ...
                for (o = 0; o <= 24; o += 8) i <<= 1, i |= e >>> o + s & 1;
            }
            for (s = 6; s >= 0; s -= 2) {
                for (o = 1; o <= 25; o += 8) a <<= 1, a |= t >>> o + s & 1;
                for (o = 1; o <= 25; o += 8) a <<= 1, a |= e >>> o + s & 1;
            }
            n[r + 0] = i >>> 0, n[r + 1] = a >>> 0;
        }, t.rip = function(e, t, n, r) {
            for (var i = 0, a = 0, s = 0; s < 4; s++) for (var o = 24; o >= 0; o -= 8) i <<= 1, 
            i |= t >>> o + s & 1, i <<= 1, i |= e >>> o + s & 1;
            for (s = 4; s < 8; s++) for (o = 24; o >= 0; o -= 8) a <<= 1, a |= t >>> o + s & 1, 
            a <<= 1, a |= e >>> o + s & 1;
            n[r + 0] = i >>> 0, n[r + 1] = a >>> 0;
            ...
            for (o = 0; o <= 24; o += 8) i <<= 1, i |= t >> o + s & 1;
            for (s = 1; s <= 3; s++) {
                for (o = 0; o <= 24; o += 8) a <<= 1, a |= t >> o + s & 1;
                for (o = 0; o <= 24; o += 8) a <<= 1, a |= e >> o + s & 1;
            }
            for (o = 0; o <= 24; o += 8) a <<= 1, a |= e >> o + s & 1;
            n[r + 0] = i >>> 0, n[r + 1] = a >>> 0;
        ...
        t.pc2 = function(e, t, n, i) {
            for (var a = 0, s = 0, o = r.length >>> 1, d = 0; d < o; d++) a <<= 1, a |= e >>> r[d] & 1;
            for (d = o; d < r.length; d++) s <<= 1, s |= t >>> r[d] & 1;
            n[i + 0] = a >>> 0, n[i + 1] = s >>> 0;
        ...
        t.substitute = function(e, t) {
            for (var n = 0, r = 0; r < 4; r++) {
                var a = e >>> 18 - 6 * r & 63, s = i[64 * r + a];
                n <<= 4, n |= s;
            }
            for (r = 0; r < 4; r++) {
                a = t >>> 18 - 6 * r & 63, s = i[256 + 64 * r + a];
                n <<= 4, n |= s;
            }
            return n >>> 0;
        };
        ...
        t.permute = function(e) {
            for (var t = 0, n = 0; n < a.length; n++) t <<= 1, t |= e >>> a[n] & 1;
            return t >>> 0;
...
// ==================== 元数据 ====================
// 此文件包含从 wx03bc60e1b437e39c 提取的 DES 算法实现
// 检测位置: 行 11585-11585
// 变量名: r
// 检测源: static
