...
function() {
            ...
            for (g = 0; g < 256; g++) {
                ...
                y = y >>> 8 ^ 255 & y ^ 99, r[v] = y;
                var m = h[i[y] = v], b = h[m], w = h[b], A = 257 * h[y] ^ 16843008 * y;
                o[v] = A << 24 | A >>> 8, a[v] = A << 16 | A >>> 16, s[v] = A << 8 | A >>> 24, u[v] = A, 
                A = 16843009 * w ^ 65537 * b ^ 257 * m ^ 16843008 * v, c[y] = A << 24 | A >>> 8, 
                f[y] = A << 16 | A >>> 16, p[y] = A << 8 | A >>> 24, d[y] = A, v ? (v = m ^ h[h[h[w ^ m]]], 
                _ ^= h[h[_]]) : v = _ = 1;
            }
...
// ==================== 元数据 ====================
// 此文件包含从 wx00f072e78f17f016 提取的 AES 算法实现
// 检测位置: 行 735-735
// 变量名: x
// 检测源: static
