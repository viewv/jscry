                ...
                try {
                    ...
                    try {
                        ...
                        if (path.parent && path.parent.type === 'ExpressionStatement') {
                            // If IIFE is an expression statement, extract the whole expression
                        ...
                        // Retrieve all traced variables from __trace object
                ...
                try {
                        ...
                        programPath.traverse({
                            ...
                            ObjectProperty(path) {
                                ...
                                if ((key.type === "Identifier" && key.name === varName) ||
                                    ...
                                    if (path.node.start < targetFunctionStart) {
                                        // Directly return property value, not the whole statement
                                        const propertyValue = path.node.value;
                                        declarations.push({
                                            type: "property",
                                            node: propertyValue,  // Return only the value node
                                            code: code.slice(propertyValue.start, propertyValue.end),  // Extract only the code of the value
                            ...
                            VariableDeclarator(path) {
                                if (path.node.id.type === "Identifier" && path.node.id.name === varName) {
                                    if (path.node.start < targetFunctionStart) {
                                        // For variable declarations, return the initialization value
                                        const initValue = path.node.init;
                                        if (initValue) {
                                            declarations.push({
                                                type: "declaration",
                                                node: initValue,  // Return initialization value node
                            ...
                            AssignmentExpression(path) {
                                ...
                                if (left.type === "Identifier" && left.name === varName) {
                                    if (path.node.start < targetFunctionStart) {
                                        // For assignment, return the right-hand value
                    ...
                    for (const varName of referencedVars) {
                        ...
                        const declarations = findVariableDeclarationsInScope(
                            ...
                            path.scope,
                            code,
                            functionNode  // Pass target function node
                        );
                        if (declarations.length > 0) {
